...
pub fn assert_storable_asset_config(asset_config: AssetConfig) {
    ...
    assert!(
        asset_config.total_collateral_shares == unpacked.total_collateral_shares,
        "total-collateral-shares-precision-loss",
    );
    ...
    assert!(
        asset_config.last_rate_accumulator == unpacked.last_rate_accumulator, "last-rate-accumulator-precision-loss",
    );
    assert!(
        asset_config.last_full_utilization_rate == unpacked.last_full_utilization_rate,
        "last-full-utilization-rate-precision-loss",
    );

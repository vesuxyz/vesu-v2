#[cfg(test)]
mod TestDefaultExtensionEKV2 {
    use snforge_std::{
        start_prank, stop_prank, start_warp, CheatTarget, get_class_hash, ContractClass, declare, prank, CheatSpan
    };
    use starknet::{get_block_timestamp, get_contract_address, contract_address_const};
    use vesu::{
        ...
        test::setup_v2::{
            setup_env_ek, create_pool_ek, TestConfigEK, deploy_assets, deploy_asset, EnvEK, EKUBO_TWAP_PERIOD,
            test_interest_rate_config
        },
        singleton_v2::{ISingletonV2DispatcherTrait}, data_model::{AssetParams, LTVParams, LTVConfig},
        extension::default_extension_ek_v2::{
            InterestRateConfig, EkuboOracleParams, LiquidationParams, IDefaultExtensionEKV2DispatcherTrait,
    ...
    #[test]
    fn test_create_pool() {
        let EnvEK { singleton, extension, config, users, .. } = setup_env_ek(
            Zeroable::zero(),
            Zeroable::zero(),
            Zeroable::zero(),
            Zeroable::zero(),
        ...
        create_pool_ek(extension, config, users.creator, Option::None);
        ...
        let TestConfigEK { pool_id, collateral_asset, debt_asset, .. } = config;
    ...
    fn test_create_pool_empty_asset_params() {
        let EnvEK { extension, users, .. } = setup_env_ek(
            Zeroable::zero(),
            Zeroable::zero(),
            Zeroable::zero(),
            Zeroable::zero(),
    ...
    fn test_create_pool_interest_rate_params_mismatch() {
        let EnvEK { extension, config, users, .. } = setup_env_ek(
            Zeroable::zero(),
            Zeroable::zero(),
            Zeroable::zero(),
            Zeroable::zero(),
    ...
    fn test_add_asset_not_owner() {
        ...
        extension.add_asset(config.pool_id, asset_params, v_token_params, interest_rate_config, ekubo_oracle_params);
    }
    ...
    fn test_add_asset_no_liquidity() {
        let EnvEK { extension, ekubo_core, ekubo_oracle, config, users, .. } = setup_env_ek(
            Zeroable::zero(),
            Zeroable::zero(),
            Zeroable::zero(),
            Zeroable::zero(),
    ...
    fn test_add_asset_ek() {
        let EnvEK { singleton, extension, ekubo_core, ekubo_oracle, config, users, .. } = setup_env_ek(
            Zeroable::zero(),
            Zeroable::zero(),
            Zeroable::zero(),
            Zeroable::zero(),
    ...
    fn test_extension_set_oracle_parameter() {
        ...
        extension
            .set_ekubo_oracle_parameter(
                config.pool_id, config.collateral_asset.contract_address, 'period', new_twap_period.into()
            );
        stop_prank(CheatTarget::One(extension.contract_address));

        let oracle_config = extension.ekubo_oracle_config(config.pool_id, config.collateral_asset.contract_address);
    ...
    fn test_extension_set_oracle_parameter_caller_not_owner() {
        ...
        extension.set_ekubo_oracle_parameter(config.pool_id, config.collateral_asset.contract_address, 'timeout', 5);
    }

    #[test]
    #[should_panic(expected: "ekubo-oracle-period-too-large")]
    fn test_extension_set_oracle_parameter_period_too_large() {
        let EnvEK { extension, config, users, ekubo_oracle, .. } = setup_env_ek(
            Zeroable::zero(),
            Zeroable::zero(),
            Zeroable::zero(),
            Zeroable::zero(),
        ...
        extension
            .set_ekubo_oracle_parameter(
                config.pool_id, config.collateral_asset.contract_address, 'period', EKUBO_TWAP_PERIOD.into()
            );
        stop_prank(CheatTarget::One(extension.contract_address));
    }
    ...
    fn test_extension_set_oracle_parameter_invalid_oracle_parameter() {
        ...
        start_prank(CheatTarget::One(extension.contract_address), users.creator);
        extension.set_ekubo_oracle_parameter(config.pool_id, config.collateral_asset.contract_address, 'a', 5);
        stop_prank(CheatTarget::One(extension.contract_address));
    }
    ...
    fn test_extension_set_oracle_parameter_oracle_config_not_set() {
        ...
        start_prank(CheatTarget::One(extension.contract_address), users.creator);
        extension.set_ekubo_oracle_parameter(config.pool_id, Zeroable::zero(), 'period', 1500);
        stop_prank(CheatTarget::One(extension.contract_address));
    }
    ...
    fn test_extension_upgrade_only_owner() {
        let EnvEK { extension, .. } = setup_env_ek(
            Zeroable::zero(),
            Zeroable::zero(),
            Zeroable::zero(),
            Zeroable::zero(),
        ...
        let new_classhash = declare("MockExtensionEKV2Upgrade").class_hash;
    ...
    fn test_extension_upgrade() {
        ...
        assert!(tag == 'MockExtensionEKV2Upgrade', "Invalid tag");
    }

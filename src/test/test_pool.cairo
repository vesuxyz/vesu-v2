...
mod TestPool {
    ...
    fn test_non_existent_asset_config() {
        let Env { pool, config, users, .. } = setup_env(Zero::zero(), Zero::zero(), Zero::zero(), Zero::zero());

        create_pool(pool, config, users.owner, users.curator, Option::None);
    ...
    fn test_create_pool_duplicate_asset() {
        ...
        pool.add_asset(params: collateral_asset_params, :interest_rate_config, :pragma_oracle_params);
        pool.add_asset(params: collateral_asset_params, :interest_rate_config, :pragma_oracle_params);
    ...
    fn test_create_pool_assert_ltv_config_invalid_ltv_config() {
        ...
        let debt_asset = deploy_asset(users.curator);
        ...
        collateral_asset.approve(pool.contract_address, INFLATION_FEE);
        cheat_caller_address(debt_asset.contract_address, users.curator, CheatSpan::TargetCalls(1));
        debt_asset.approve(pool.contract_address, INFLATION_FEE);

        start_cheat_caller_address(pool.contract_address, users.curator);
        pool.add_asset(params: collateral_asset_params, :interest_rate_config, :pragma_oracle_params);
        pool.add_asset(params: debt_asset_params, :interest_rate_config, :pragma_oracle_params);
        stop_cheat_caller_address(pool.contract_address);
    ...
    fn test_add_asset_not_curator() {
        ...
        let pragma_oracle_params = dummy_pragma_oracle_params();
        let interest_rate_config = dummy_interest_rate_config();
        pool.add_asset(asset_params, :interest_rate_config, :pragma_oracle_params);
    }
    ...
    fn test_set_asset_parameter_fee_shares() {
        ...
        start_cheat_caller_address(pool.contract_address, users.curator);
        pool.set_asset_parameter(debt_asset.contract_address, 'fee_rate', SCALE);
        stop_cheat_caller_address(pool.contract_address);
    ...
    fn test_pool_call_paused_contract() {
        ...
        cheat_caller_address(pool.contract_address, users.owner, CheatSpan::TargetCalls(1));
        pool.pause();
        assert!(pool.is_paused(), "Contract should be paused");
    ...
    fn test_pool_pause_flow() {
        ...
        assert!(pool.is_paused(), "Contract should be paused");
        pool.unpause();
        assert!(!pool.is_paused(), "Contract should be unpaused");
        stop_cheat_caller_address(pool.contract_address);
    ...
    #[test]
    #[should_panic(expected: ('Caller is not the owner',))]
    fn test_pool_upgrade_only_owner() {
        let Env { pool, .. } = setup_env(Zero::zero(), Zero::zero(), Zero::zero(), Zero::zero());
        let new_classhash = *declare("MockPoolUpgrade").unwrap().contract_class().class_hash;
        start_cheat_caller_address(pool.contract_address, contract_address_const::<'not_owner'>());
        pool.upgrade(new_classhash, Option::None);
    }

    #[test]
    fn test_pool_upgrade() {
        let Env { pool, users, .. } = setup_env(Zero::zero(), Zero::zero(), Zero::zero(), Zero::zero());
        let new_classhash = *declare("MockPoolUpgrade").unwrap().contract_class().class_hash;
        cheat_caller_address(pool.contract_address, users.owner, CheatSpan::TargetCalls(1));
        pool.upgrade(new_classhash, Option::None);
        let tag = IMockPoolUpgradeDispatcher { contract_address: pool.contract_address }.tag();
        assert!(tag == 'MockPoolUpgrade', "Invalid tag");
    }

    #[test]
    #[should_panic(expected: ('invalid upgrade name',))]
    fn test_pool_upgrade_wrong_name() {
        let Env { pool, users, .. } = setup_env(Zero::zero(), Zero::zero(), Zero::zero(), Zero::zero());
        let new_classhash = *declare("MockPoolUpgradeWrongName").unwrap().contract_class().class_hash;
        cheat_caller_address(pool.contract_address, users.owner, CheatSpan::TargetCalls(1));
        pool.upgrade(new_classhash, Option::None);
    }

    #[test]
    fn test_pool_upgrade_eic() {
        let Env { pool, users, .. } = setup_env(Zero::zero(), Zero::zero(), Zero::zero(), Zero::zero());
        let new_classhash = *declare("MockPoolUpgrade").unwrap().contract_class().class_hash;
        let eic_classhash = *declare("MockEIC").unwrap().contract_class().class_hash;
        let old_pool_name = 'PoolName';
        assert!(pool.pool_name() == old_pool_name, "Old pool name mismatch");
        let new_pool_name = 'NewPoolName';
        cheat_caller_address(pool.contract_address, users.owner, CheatSpan::TargetCalls(1));
        pool.upgrade(new_classhash, Some((eic_classhash, array![new_pool_name].span())));
        let actual_new_pool_name = pool.pool_name();
        assert!(actual_new_pool_name == new_pool_name, "New pool name mismatch");
    }

    #[test]
    #[should_panic(expected: 'Index out of bounds')]
    fn test_pool_upgrade_eic_initialize_failed() {
        let Env { pool, users, .. } = setup_env(Zero::zero(), Zero::zero(), Zero::zero(), Zero::zero());
        let new_classhash = *declare("MockPoolUpgrade").unwrap().contract_class().class_hash;
        let eic_classhash = *declare("MockEIC").unwrap().contract_class().class_hash;
        cheat_caller_address(pool.contract_address, users.owner, CheatSpan::TargetCalls(1));
        pool.upgrade(new_classhash, Some((eic_classhash, array![].span())));
    }

    #[test]
    #[should_panic(expected: 'Invalid mock eic data')]
    fn test_pool_upgrade_eic_invalid_data() {
        let Env { pool, users, .. } = setup_env(Zero::zero(), Zero::zero(), Zero::zero(), Zero::zero());
        let new_classhash = *declare("MockPoolUpgrade").unwrap().contract_class().class_hash;
        let eic_classhash = *declare("MockEIC").unwrap().contract_class().class_hash;
        cheat_caller_address(pool.contract_address, users.owner, CheatSpan::TargetCalls(1));
        pool.upgrade(new_classhash, Some((eic_classhash, array!['invalid-mock-eic-data'].span())));
    }

    #[test]
    fn test_pool_change_owner() {
        let Env { pool, users, .. } = setup_env(Zero::zero(), Zero::zero(), Zero::zero(), Zero::zero());
        let ownable_dispatcher = IOwnableTwoStepDispatcher { contract_address: pool.contract_address };
        ...
        start_cheat_caller_address(pool.contract_address, new_owner);

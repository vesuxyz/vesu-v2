...
mod TestSingletonV2 {
    ...
    fn test_set_asset_parameter_fee_shares() {
        ...
        let (fee_shares_after, _) = singleton.get_fees(pool_id, debt_asset.contract_address);
        assert!(fee_shares_after == fee_shares_before, "Fee shares mismatch");

        let asset_config = singleton.asset_config(config.pool_id, debt_asset.contract_address);
        assert!(asset_config.fee_rate == SCALE, "Fee rate not set");
    }

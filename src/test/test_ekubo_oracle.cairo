...
mod TestEkuboOracle {
    ...
    use vesu::{
        ...
        test::{
            setup_v2::{create_pool_ek, deploy_asset_with_decimals, setup_env_ek, TestConfigEK, EnvEK},
        ...
        extension::{
            ...
            default_extension_ek_v2::{IDefaultExtensionEKV2DispatcherTrait, IDefaultExtensionEKV2Dispatcher,}
        },
        math::pow_10, vendor::erc20::{ERC20ABIDispatcher as IERC20Dispatcher, ERC20ABIDispatcherTrait},
    };

    fn setup_env_ek_with_asset_decimals(
    ...
    fn test_get_ekubo_price_quote_asset_less_decimals() {
        ...
        let EnvEK { extension, config, users, .. } = setup_env_ek_with_asset_decimals(
            collateral_asset_decimals, debt_asset_decimals, third_asset_decimals, quote_asset_decimals
        );
        let TestConfigEK { collateral_asset, debt_asset, third_asset, quote_asset, .. } = config;
        let default_extension_ek = extension;

        create_pool_ek(extension, config, users.creator, Option::None);
        let TestConfigEK { pool_id, .. } = config;
        ...
        let third_asset_price = extension_dispatcher.price(pool_id, third_asset.contract_address);

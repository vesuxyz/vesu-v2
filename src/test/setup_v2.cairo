...
pub fn setup_env(
    ...
    let singleton = ISingletonV2Dispatcher {
        contract_address: deploy_with_args(
            "SingletonV2",
            array!['PoolName', users.owner.into(), users.extension_owner.into(), oracle.contract_address.into()],
        ),
    };
...
pub fn setup_pool(
    ...
    create_pool(oracle, singleton, config, users.owner, users.extension_owner, interest_rate_config);

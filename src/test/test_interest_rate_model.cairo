...
mod TestInterestRateModel {
    ...
    fn test_utilization() {
        ...
        assert!(to_apr(rate) == 07_515, "invalid rate");
        assert!(to_apr(next_full_utilization_rate) == 19_999, "invalid next_full_utilization_rate");
    }
    ...
    fn test_set_interest_rate_model_fee_shares() {
        ...
        start_cheat_caller_address(singleton.contract_address, users.curator);
        singleton.set_interest_rate_parameter(debt_asset.contract_address, 'max_target_utilization', 88_000);
        stop_cheat_caller_address(singleton.contract_address);

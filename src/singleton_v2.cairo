...
pub trait ISingletonV2<TContractState> {
    ...
    fn add_asset(
        ref self: TContractState,
        params: AssetParams,
        interest_rate_config: InterestRateConfig // pragma_oracle_params: PragmaOracleParams,
    );
    fn set_ltv_config(
...
mod SingletonV2 {
    ...
    impl SingletonV2Impl of ISingletonV2<ContractState> {
        ...
        fn add_asset(
            ...
            let oracle = IOracleDispatcher { contract_address: self.oracle.read() };
            assert!(oracle.price(params.asset).value.is_non_zero(), "oracle-price-not-set");

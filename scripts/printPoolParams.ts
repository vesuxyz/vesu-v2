...
for (const pool of pools) {
  const pool_data = await Promise.all([
    pool.pool_name(),
    pool.oracle(),
    pool.owner(),
    pool.fee_recipient(),
    pool.curator(),
    pool.pending_curator(),
    pool.pausing_agent(),
    pool.is_paused(),
  ]);
  console.log("Pool name:                 ", shortString.decodeShortString(pool_data[0]));
  console.log("Oracle:                    ", toAddress(pool_data[1]));
  console.log("Owner:                     ", toAddress(pool_data[2]));
  console.log("Fee recipient:             ", toAddress(pool_data[3]));
  console.log("Curator:                   ", toAddress(pool_data[4]));
  console.log("Pending curator:           ", toAddress(pool_data[5]));
  console.log("Pausing agent:             ", toAddress(pool_data[6]));
  console.log("Paused:                    ", pool_data[7]);
  console.log("");
